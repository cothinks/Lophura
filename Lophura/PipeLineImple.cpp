...
PipeLineImple::PipeLineImple(void) : transformed_verts(800*600)
{
}


PipeLineImple::~PipeLineImple(void)
{
}

void PipeLineImple::Lanche( RenderStatePtr state )
{
	state_ = state;
...
void PipeLineImple::Prepare()
{
	BufferPtr vertex_buffer = state_->vertex_buffer_;
	Matrix44  wvp			= state_->wrold_view_proj_;

	transformed_verts	= *vertex_buffer;

	vec4*	vertx		= reinterpret_cast<vec4*>(transformed_verts.RawData(0));
...
void PipeLineImple::Draw()
{
	...
	for (size_t i = 0;i < index_count; i += 3)
	{
		...
		//state_->swap_chain_->Present();
	}
	//{
	//	int x = vertx[i].x();
	//	int y = vertx[i].y();

	//	COLORRGBA32F color(1.0f,1.0f,1.0f,1.0f);

	//	color_target->SetPos(x,y,color);
	//}
